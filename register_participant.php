...
$check_stmt = $conn->prepare("SELECT status, cancellation_reason, rejection_reason, removal_reason, proof_path, signature_path FROM participants WHERE election_id = ? AND user_id = ?");
...
// Backend Enforcement: Registration Phase Check
if ($election['registration_status'] !== 'open') {
    if (!$existing || $existing['status'] !== 'Approved') {
        header("Location: student_dashboard.php?error=registration_closed");
        exit();
    }
}

// Auto-fix: Ensure removal columns exist in participants table
$res = $conn->query("SHOW COLUMNS FROM participants LIKE 'position'");
if ($res->num_rows == 0) {
    $conn->query("ALTER TABLE participants ADD COLUMN position VARCHAR(100) DEFAULT NULL");
}
...
$reg_ended = ($election['registration_status'] === 'closed' && (time() >= strtotime($election['registration_end']) || $election['status'] === 'Registration Ended' || $election['status'] === 'Voting Active' || $election['status'] === 'Completed'));

$error = '';
$success = '';

if ($_SERVER['REQUEST_METHOD'] == 'POST' && $is_reg_open) {
    if ($existing && $existing['status'] !== null) {
        $error = "You have already submitted your participation request.";
    } else {
        ...
        if (!$rules_accepted) {
            ...
            strcasecmp($entered_name, $db_name) !== 0 ||
            ...
            (($dob_date = new DateTime($dob)) && ($today = new DateTime('2026-03-18')) && (($age = $today->diff($dob_date)->y) < 17 || $age > 30 || $dob_date->format('Y') < 1900))
        ) {
            ...
            if (!in_array(strtolower($photo_ext), $allowed) || !in_array(strtolower($proof_ext), $allowed) || !in_array(strtolower($sig_ext), $allowed)) {
                ...
                if (
                    ...
                    if ($ins->execute()) {
                        $success = "Participation Registration Completed Successfully.";
...
<body>
                ...
                <div class="status-banner">
                    ...
                    <p>
                        ...
                        if ($existing['status'] === 'Pending')
                            echo 'You have already submitted your participation request.<br>Your registration is under review.';
                        elseif ($existing['status'] === 'Removed by Admin')
                            echo '<span style="color:#dc2626; font-weight:600;">Your participation has been removed by the administration.</span><br><small>Reason: ' . htmlspecialchars($existing['removal_reason'] ?? 'N/A') . '</small>';
                        elseif ($existing['status'] === 'Cancelled')
                            echo 'Your participation has been cancelled.';
                        else
                    ...
                    <div style="margin-top: 1.5rem; display: flex; gap: 1rem; justify-content: center;">
                        <a href="student_dashboard.php" class="btn btn-secondary"
                            style="padding: 1rem 2rem; border-radius: 1rem;">Back to Dashboard</a>
                    </div>
                </div>
                ...
                <form method="POST" enctype="multipart/form-data" id="regForm" autocomplete="off">
                    ...
                    <div class="form-grid">
                        <div class="form-section">
                            ...
                            <div class="identity-display-field"><?php echo $db_college_id; ?></div>
                            <input type="hidden" name="college_id" value="<?php echo $db_college_id; ?>">
                        </div>
                        <div class="form-section">
                            <label class="form-label">Department</label>
                            <div class="identity-display-field"><?php echo $db_dept; ?></div>
                            <input type="hidden" name="department" value="<?php echo $db_dept; ?>">
                        </div>
                    </div>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-section">
                            ...
                            <input type="date" name="dob" id="dobInput" class="form-control" required onchange="validateDOB()" oninput="validateDOB()">
                            <small id="dobError" style="color: #dc2626; font-size: 0.8rem; display: none; margin-top: 5px;">Please enter a valid Date of Birth.</small>
                        </div>
                    </div>

                    <div class="form-section">
                        <label class="form-label">Year of Study</label>
                        <div class="identity-display-field"><?php echo $db_year; ?> Year</div>
                        <input type="hidden" name="year" value="<?php echo $db_year; ?>">
                    </div>
                    ...
                    <div class=" form-section">
                        ...
                        <div style="display: flex; gap: 1rem; align-items: flex-start;">
                            ...
                            <div style="flex-grow: 1;">
                                ...
                                <small style="color: #64748b; display: block; margin-top: 5px;">Must be a clear headshot.
                                    Max 2MB.</small>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <input type="file" name="proof_file" class="form-control" accept=".jpg,.jpeg,.png" required
                            id="proofInput">
                        <small style="color: #64748b; display: block; margin-top: 5px;">Upload a clear scan of your
                            College
    ...
    <script>
        ...
        function validateDOB() {
            ...
            const today = new Date('2026-03-18'); // system time
            ...
            // Valid range: 17 to 30 years old. Year must be at least 1900.
            const isValid = (age >= 17 && age <= 30 && dobDate.getFullYear() >= 1900);
