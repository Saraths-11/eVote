...
<body>
    ...
    <main class="main-content">
        ...
        <?php switch ($current_tab):
            ...
            case 'dashboard':
                // Fetch stats
                $active_elec = $conn->query("SELECT COUNT(*) as count FROM elections WHERE status = 'Voting Active'")->fetch_assoc()['count'];
                ...
                $completed_elec = $conn->query("SELECT COUNT(*) as count FROM elections WHERE status = 'Completed'")->fetch_assoc()['count'];
                ?>
            ...
            case 'monitor':
                ...
                $elections = $conn->query("SELECT id, title, eligible_voters FROM elections WHERE status IN ('Voting Active', 'Upcoming')");
                ?>
                <div class="page-header">
                    ...
                    <div class="content-card">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
                            ...
                            <div style="text-align: right;">
                                <span style="font-size: 0.9rem; color: #64748b;">Eligible Voters:
                                    <strong><?php echo $elec['eligible_voters']; ?></strong></span>
                            </div>
                        </div>
                        <?php
                        $eid = $elec['id'];
                        ...
                        $eligible_voters = $elec['eligible_voters'];
                            ...
                            <?php while ($cand = $cands->fetch_assoc()):
                                ...
                                <div style="background: #f9fafb; padding: 1.25rem; border-radius: 12px; border: 1px solid #e5e7eb;">
                                    ...
                                    <div style="text-align: right; font-size: 0.75rem; color: #6b7280; margin-top: 0.25rem;">
                                        <?php echo $percent; ?>% of eligible voters
                                    </div>
                                </div>
                            <?php endwhile; ?>
                        </div>
            ...
            // -----------------------------------
            case 'results':
                $elections = $conn->query("SELECT id, title, eligible_voters FROM elections WHERE status = 'Completed' AND is_published = 1");
                ?>
                <div class="page-header">
            ...
            // -----------------------------------
            case 'past':
                $result = $conn->query("SELECT * FROM elections WHERE status = 'completed'");
                ?>
                <div class="page-header">
