...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($title) || empty($description) || empty($reg_start) || empty($reg_end) || empty($ele_start) || empty($ele_end)) {
        $error = "All fields are required.";
    } elseif ($ts_reg_start < $now || $ts_reg_end < $now || $ts_ele_start < $now || $ts_ele_end < $now) {
        ...
        $status = 'Upcoming';
        if ($now >= $ts_reg_start && $now < $ts_reg_end) {
            $status = 'Registration Open';
        } elseif ($now >= $ts_reg_end && $now < $ts_ele_start) {
            $status = 'Registration Ended';
        } elseif ($now >= $ts_ele_start && $now < $ts_ele_end) {
            $status = 'Voting Active';
        } elseif ($now >= $ts_ele_end) {
            $status = 'Completed';
        }

        // Insert into Database
        $stmt = $conn->prepare("INSERT INTO elections (title, description, eligible_voters, registration_start, registration_end, election_start, election_end, status, registration_status, voting_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'closed', 'not_started')");
        if ($stmt) {
            $stmt->bind_param("ssisssss", $title, $description, $eligible_voters, $reg_start, $reg_end, $ele_start, $ele_end, $status);
            if ($stmt->execute()) {
...
$current_dt = "2026-01-09T11:30"; // Based on system metadata
...
<body>
        ...
        <form action="" method="POST" id="electionForm" onsubmit="return validateForm()">
            <div class="card">
                ...
                <div class="form-section">
                    ...
                    <div class="info-card">
                        <span>⚠️</span>
                        <p>Voting will be enabled automatically during this window. Election start must be after
                            registration end (usually on a previous day).</p>
                    </div>
                </div>
    ...
    <script>
        ...
        const inputs = ['registration_start', 'registration_end', 'election_start', 'election_end'];
        ...
        document.getElementById('election_end').addEventListener('change', function () {
        ...
        function validateForm() {
            ...
            // 1. Past dates check
            if (regStart < currentTime || regEnd < currentTime || eleStart < currentTime || eleEnd < currentTime) {
            ...
            if (eleStart <= regEnd) {
                alert("The election can only start after participant registration has ended.");
                return false;
            }
