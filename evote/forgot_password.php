...
<body>
    ...
    <script type="module">
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                // 2. Check if email is registered in MySQL
                ...
                if (!data.exists) {
                    showStatus("This email is not registered. Please Sign Up first.", "error");
                    submitBtn.innerText = "Send Reset Link";
                    submitBtn.disabled = false;
                    return;
                }

                // 3. Email exists -> Send Firebase Reset Email
                submitBtn.innerText = "Sending...";

                await sendPasswordResetEmail(auth, email);

                showStatus("Success! Check your email for the reset link.", "success");
                submitBtn.innerText = "Sent";
                form.reset();

                // Optional: Re-enable after a few seconds or leave as sent
                setTimeout(() => {
                    submitBtn.disabled = false;
                    submitBtn.innerText = "Send Reset Link";
                }, 5000);
                ...
                if (error.code) { // Firebase errors
                    if (error.code === 'auth/user-not-found') {
                        msg = "User not found in authentication system. (Legacy account? Contact Admin).";
