...
$is_faculty = ($_SESSION['role'] === 'faculty');
...
$res = $conn->query("SHOW COLUMNS FROM participants LIKE 'cancellation_reason'");
if ($res->num_rows == 0) {
    $conn->query("ALTER TABLE participants ADD COLUMN cancellation_reason TEXT DEFAULT NULL");
}
$res = $conn->query("SHOW COLUMNS FROM participants LIKE 'cancelled_at'");
if ($res->num_rows == 0) {
    $conn->query("ALTER TABLE participants ADD COLUMN cancelled_at DATETIME DEFAULT NULL");
}
...
if ($is_admin && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($participant_id) {
        // Fetch election status for this participant
        $status_check_stmt = $conn->prepare("SELECT e.voting_status FROM participants p JOIN elections e ON p.election_id = e.id WHERE p.id = ?");
        $status_check_stmt->bind_param("i", $participant_id);
        $status_check_stmt->execute();
        $ele_status = $status_check_stmt->get_result()->fetch_assoc()['voting_status'] ?? 'not_started';

        if (($action === 'Removed' || $action === 'Approved') && ($ele_status === 'active' || $ele_status === 'ended')) {
            header("Location: view_participants.php?error=voting_started&view=" . ($_GET['view'] ?? 'Approved'));
            exit();
        }
        ...
        if ($action === 'Cancelled') {
            ...
            $stmt = $conn->prepare("UPDATE participants SET status = 'Removed by Admin', removal_reason = ?, removed_at = NOW() WHERE id = ?");
            $stmt->bind_param("si", $reason, $participant_id);
        }
...
$allowed_views = ['Pending', 'Approved', 'Cancelled', 'Removed', 'Rejected', 'All'];
...
// Fetch participants
$sql = "SELECT p.*, e.title as election_title, e.voting_status FROM participants p 
        JOIN elections e ON p.election_id = e.id ";

$where = [];

if ($current_view !== 'All') {
    $status_to_query = ($current_view === 'Removed') ? 'Removed by Admin' : $current_view;
    $where[] = "p.status = '" . $conn->real_escape_string($status_to_query) . "'";
}
...
<body>
    ...
    <div class="dashboard-content">
        ...
        <?php if (isset($_GET['error']) && $_GET['error'] === 'voting_started'): ?>
            <div class="alert alert-error" style="background: #FEF2F2; color: #991B1B; border: 1px solid #FEE2E2; padding: 1rem; border-radius: 12px; margin-bottom: 2rem;">
                ⚠️ <strong>Action Blocked:</strong> Candidates cannot be removed after the voting phase has started.
            </div>
        <?php endif; ?>
        <div class="tab-nav">
            <?php foreach (['Pending', 'Approved', 'Cancelled', 'Removed', 'Rejected', 'All'] as $view): ?>
                <a href="?view=<?php echo $view; ?><?php echo $election_id ? "&id=$election_id" : ""; ?>"
                    class="tab-btn <?php echo $current_view === $view ? 'active' : ''; ?>">
                    <?php echo $view; ?>
                </a>
            <?php endforeach; ?>
        </div>
                            ...
                            <td>
                                <?php if ($row['status'] == 'Pending'): ?>
                                    <div style="display: flex; gap: 0.5rem;">
                                        ...
                                        <?php else: ?>
                                            <div style="font-size: 0.75rem; color: #94a3b8; font-weight: 600;">Locked (Voting Active)</div>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <?php elseif ($row['status'] == 'Removed by Admin'): ?>
                                    <div
                                        style="font-size: 0.8rem; color: #B91C1C; background: #FEF2F2; padding: 0.5rem 0.75rem; border-radius: 8px; max-width: 200px;">
                                        <strong>Removal Reason:</strong><br>
                                        <?php echo htmlspecialchars($row['removal_reason'] ?? 'N/A'); ?>
                                        <div style="font-size: 0.7rem; margin-top: 0.4rem; opacity: 0.8;">
                                            On <?php echo date('M d, Y', strtotime($row['removed_at'])); ?>
                                        </div>
                                    </div>
                                <?php elseif ($row['status'] == 'Approved'): ?>
                                    <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                                        <div style="font-size: 0.75rem; color: #10B981; font-weight: 700;">Candidate Active</div>
                                        <?php if ($is_admin): ?>
                                            <?php if ($row['voting_status'] === 'not_started'): ?>
                                                <form method="POST"
                                                    onsubmit="return handleAction(this, 'Removed', 'Admin Action: Provide a mandatory reason for removing this candidate:');">
                                                    <input type="hidden" name="participant_id" value="<?php echo $row['id']; ?>">
                                                    <input type="hidden" name="action" value="Removed">
                                                    <button type="submit" class="btn btn-sm"
                                                        style="background: #FEE2E2; color: #B91C1C; border: 1px solid #FECACA; padding: 0.4rem 0.8rem; border-radius: 6px; cursor: pointer; font-weight: 600; font-size: 0.75rem;">
                                                        Remove Candidate
                                                    </button>
                                                </form>
                                            <?php else: ?>
                                                <div style="font-size: 0.7rem; color: #64748b; font-weight: 600; background: #f1f5f9; padding: 0.3rem 0.6rem; border-radius: 4px;">
                                                    🔒 Locked (Voting)
                                                </div>
                                            <?php endif; ?>
                                        <?php endif; ?>
                                ...
                                <?php elseif ($row['status'] == 'Cancelled'): ?>
                                    <div style="font-size: 0.8rem; color: #475569; background: #f8fafc; padding: 0.8rem; border-radius: 10px; border: 1px solid #e2e8f0; max-width: 250px;">
                                        <div style="color: #ef4444; font-weight: 800; font-size: 0.7rem; text-transform: uppercase; margin-bottom: 0.4rem; display: flex; align-items: center; gap: 0.3rem;">
                                            <span>🚫</span> Withdrawn by Student
                                        </div>
                                        <div style="color: #1e293b; font-weight: 600; margin-bottom: 0.3rem;">
                                        ...
                                        <div style="font-size: 0.7rem; color: #64748b; font-family: monospace;">
                                            📅 <?php echo $row['cancelled_at'] ? date('M d, Y - h:i A', strtotime($row['cancelled_at'])) : 'N/A'; ?>
                                        </div>
                                    </div>
                                <?php else: ?>
    ...
    <script>
        function handleAction(form, action, message) {
            ...
            if (reason === null || reason.trim() === '') {
                if (action === 'Removed') alert("Removal reason is mandatory.");
                return false;
            }
